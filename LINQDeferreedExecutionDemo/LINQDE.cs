        ...
        private static void Main()
        {
             ...
             * The LINQ queries are executed in two different ways as follows.
               ...
               Based on the above two types of execution, the LINQ operators are divided into 2 categories. They are as follows:
                    ...
                    Deferred or Lazy Operators:  These query operators are used for deferred execution. For example – select, SelectMany, where, Take, Skip, etc. belong
                    to Deferred or Lazy Operators category.
                    Immediate or Greedy Operators: These query operators are used for immediate execution. For Example – count, average, min, max, First, Last, ToArray, ToList, 
                    etc. belong to the Immediate or Greedy Operators category.
               LINQ Deferred Execution:
               In this case, the LINQ Query is not executed at the point of its declaration. That means, when we write a LINQ query, it doesn’t execute by itself. 
               It executes only when we access the query results. So, here the execution of the query is deferred until the query variable is iterated over using foreach loop.
            ...
            // The LINQ query is actually executed when we iterate thrughthe result, using a foreach loop
             ...
             *  Advantages of Deferred Execution:
                ...
                Immediate Execution
                In the case of Immediate Execution, the LINQ query is executed at the point of its declaration. So, it forces the query to execute and gets the result immediately.
            ...
            IEnumerable<Employee> result1 = (from emp in listEmployees
                where emp.Salary == 80000
                select emp).ToList();
